...
function core_hmac_md5(c, f) {
    ...
    const a = Array(16), d = Array(16);
    for (let b = 0; b < 16; b++) {
        a[b] = e[b] ^ 909522486;
        d[b] = e[b] ^ 1549556828;
    }
    const g = core_md5(a.concat(str2binl(f)), 512 + f.length * chrsz);
...
function binl2str(c) {
    let d = "";
    const a = (1 << chrsz) - 1;
    for (let b = 0; b < c.length * 32; b += chrsz) {
...
function binl2hex(c) {
    const b = hexcase ? "0123456789ABCDEF" : "0123456789abcdef";
    let d = "";
...
function binl2b64(d) {
    ...
    let f = "";
    for (let b = 0; b < d.length * 4; b += 3) {
        const e = (((d[b >> 2] >> 8 * (b % 4)) & 255) << 16) | (((d[b + 1 >> 2] >> 8 * ((b + 1) % 4)) & 255) << 8) | ((d[b + 2 >> 2] >> 8 * ((b + 2) % 4)) & 255);
        for (let a = 0; a < 4; a++) {
    ...
    const J = function (o, E) {
        ...
        if (J.bSSGte === undefined) {
            ...
            const t = function (w, m) {
                let T = [], A = 0x0, C, b = '', W = '';
                w = Y(w);
                for (let R = 0x0, v = w.length; R < v; R++) {
