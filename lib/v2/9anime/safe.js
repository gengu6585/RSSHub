...
function safe_add(x, y) {
    ...
    /** @type {number} */
    const c = (x & 65535) + (y & 65535);

    /** @type {number} */
    const len = (x >> 16) + (y >> 16) + (c >> 16);
...
function binl2b64(binarray) {
    ...
    /** @type {string} */
    const raw_composed_type = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    ...
    for (; i < binarray.length * 4; i = i + 3) {
        ...
        /** @type {number} */
        const e = (binarray[i >> 2] >> 8 * (i % 4) & 255) << 16 | (binarray[i + 1 >> 2] >> 8 * ((i + 1) % 4) & 255) << 8 | binarray[i + 2 >> 2] >> 8 * ((i + 2) % 4) & 255;
...
const test = hex_md5("1673285659000");
console.log(test);
