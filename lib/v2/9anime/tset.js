...
request = function () {
    const list = {"page": window.page,};
    $.ajax({
        url: window.url, dataType: "json", async: false, data: list, type: "GET", beforeSend (request) {
        }, success (data) {
            datas = data.info;
            $('.number').text('').append(datas);
            const j_key = '.' + hex_md5(btoa(data.key + data.value).replace(/=/g, ''));
